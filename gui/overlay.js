...
overlay.prototype.macroExport = function() {
    ...
    var sizeTable = build.table([
        [
            build.input('number', svg.svgWidth, {
                'id': 'anigenInputWidth',
                'min': '0',
                'step': 1,
                'title': 'Width (px)',
            ...
            ratioButton,
            build.input('number', svg.svgHeight, {
                'id': 'anigenInputHeight',
                'min': '0',
                'step': 1,
                'title': 'Height (px)',
...
overlay.prototype.macroEdit = function(target) {
    ...
    this.addButtonCancel();
    // add tabindex to the text inputs in the form
    allInputs = this.container.querySelectorAll("input");
    for (var n = 0; n < allInputs.length; n++) {
        if (allInputs[n].type == "text")
            allInputs[n].setAttribute("tabindex", n + 1);
    }
    this.show();
}
