...
function infoEditor(timeline) {
    ...
    // changed order of buttons: on load show play button 
    // added setState to solve the only one use error
    this.pauseButton = new uiButton(
        ['play_arrow', 'pause'], ['svg.pauseToggle(true);this.setState(0);', 'svg.pauseToggle(false);this.setState(1);'], ['Pause animations', 'Unpause animations']
    );
